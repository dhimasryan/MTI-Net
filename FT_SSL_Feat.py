# ===================================================================================================
# Original code/ Main framework was written by Yamagishi Laboratory, National Institute of Informatics
# Multi-task code was written by Ryandhimas Zezario
# ====================================================================================================
...
class MosPredictor(nn.Module):
    ...
    def __init__(self, ssl_model, ssl_out_dim):
        ...
        self.output_layer_wer = nn.Linear(self.ssl_features, 1)
        self.output_layer_intell = nn.Linear(self.ssl_features, 1)
        self.output_layer_stoi = nn.Linear(self.ssl_features, 1)        
    ...
    def forward(self, wav):
        ...
        x = res['x']
        x = torch.mean(x, 1)
        x_wer = self.output_layer_wer(x)
        x_intell = self.output_layer_intell(x)
        x_stoi = self.output_layer_stoi(x)        
        
        return x_wer.squeeze(1), x_intell.squeeze(1), x_stoi.squeeze(1), res
...
class MyDataset(Dataset):
    def __init__(self, wavdir, mos_list):
        ...
        for line in f:
            ...
            stoi = float(parts[3])            
            self.wer_lookup[wavname] = wer
            self.intell_lookup[wavname]=intell
            self.stoi_lookup[wavname]=stoi            

        self.wavdir = wavdir
        self.wavnames = sorted(self.wer_lookup.keys())
    ...
    def __getitem__(self, idx):
        ...
        score_wer = self.wer_lookup[wavname]
        score_intell = self.intell_lookup[wavname]
        score_stoi = self.stoi_lookup[wavname]       
        return wav, score_wer, score_intell, score_stoi, wavname
    ...
    def collate_fn(self, batch):  ## zero padding
        wavs, scores_wer, scores_intell, scores_stoi, wavnames = zip(*batch)
        wavs = list(wavs)
        ...
        scores_wer  = torch.stack([torch.tensor(x) for x in list(scores_wer)], dim=0)
        scores_intell  = torch.stack([torch.tensor(x) for x in list(scores_intell)], dim=0)
        scores_stoi  = torch.stack([torch.tensor(x) for x in list(scores_stoi)], dim=0)        
        return output_wavs, scores_wer, scores_intell, scores_stoi, wavnames
...
def main():
    ...
    parser.add_argument('--fairseq_base_model', default='/pre-trained_model/wav2vec_small.pt', type=str, help='Path to pretrained fairseq base model')
    parser.add_argument('--finetune_from_checkpoint', type=str, required=False, help='Path to your checkpoint to finetune from')
    parser.add_argument('--outdir', type=str, required=False, default='FT_Model', help='Output directory for your trained checkpoints')
    ...
    trainlist = os.path.join(datadir, '/Lists/train_mos_list.txt')
    validlist = os.path.join(datadir, '/Lists/val_mos_list.txt')
    ...
    for epoch in range(1,1001):
        ...
        for i, data in enumerate(tqdm(trainloader), 0):
            ...
            inputs, labels_wer, labels_intell, labels_stoi, filenames = data
            inputs = inputs.to(device)
            labels_wer = labels_wer.to(device)
            labels_intell = labels_intell.to(device)
            labels_stoi = labels_stoi.to(device)            

            optimizer.zero_grad()
            output_wer, output_intell, output_stoi,_ = net(inputs)
            loss_wer = criterion(output_wer, labels_wer)
            loss_intell = criterion(output_intell, labels_intell)
            loss_stoi = criterion(output_stoi, labels_stoi)            
            loss = loss_wer + loss_intell + loss_stoi
